...
var (
	ZLength = casting_machine.ZLength
	ZStep   = casting_machine.ZStep
	Width   = casting_machine.Width
	YStep   = casting_machine.YStep
	Length  = casting_machine.Length
	XStep   = casting_machine.XStep
)
...
type calculatorWithArrDeque struct {
	...
	castingMachine *casting_machine.CastingMachine // 铸机

	steel1 *steel_type.Steel // 第一种钢种
	steel2 *steel_type.Steel // 第二种钢种
...
func NewCalculatorWithArrDeque() *calculatorWithArrDeque {
	...
	// 初始化铸机
	c.castingMachine = casting_machine.NewCastingMachine(1)
...
func (c *calculatorWithArrDeque) InitCastingMachine(castingMachineNumber int) {
	c.castingMachine = casting_machine.NewCastingMachine(castingMachineNumber)
}

func (c *calculatorWithArrDeque) GetCastingMachine() *casting_machine.CastingMachine {
	return c.castingMachine
}

func (c *calculatorWithArrDeque) InitSteel(steelValue int, castingMachine *casting_machine.CastingMachine) {
	if c.runningState == stateRunning { // 如果此时有其他钢种正在计算，只有当拉尾坯模式将前一个铸坯全部移除铸机后，isRunning状态才会变为false
		...
		// 还未运行
		c.steel1 = steel_type.NewSteel(steelValue, castingMachine)
	}
}

func (c *calculatorWithArrDeque) getParameter(z int) *steel_type.Parameter {
...
func (c *calculatorWithArrDeque) calculateTimeStep() (float32, time.Duration) {
	...
	min := float32(1000.0)
	var t float32
	var parameter *steel_type.Parameter
	c.Field.Traverse(func(z int, item *casting_machine.ItemType) {
		// 跳过为空的切片
		if item[0][0] == -1 {
			return
		}
	...
	for {
		...
		default:
			if c.Field.Size() == 0 { // 计算时间等于0，意味着还没有切片产生，此时可以等待产生一个切片再计算
				duration += casting_machine.OneSliceDuration
				gap = casting_machine.OneSliceDuration
				deltaT = float32(casting_machine.OneSliceDuration.Seconds())
			} else {
...
// 计算一个left top点的温度变化
func (c *calculatorWithArrDeque) calculatePointLT(deltaT float32, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHlt = deltaHlt * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	...
	if c.alternating {
		c.thermalField1.Set(z, Width/YStep-1, 0, slice[Width/YStep-1][0]-deltaHlt*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	} else {
		// 需要修改焓的变化到温度变化k映射关系
		c.thermalField.Set(z, Width/YStep-1, 0, slice[Width/YStep-1][0]-deltaHlt*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	}
}

// 计算上表面点温度变化
func (c *calculatorWithArrDeque) calculatePointTA(deltaT float32, x, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHta = deltaHta * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	...
	if c.alternating {
		c.thermalField1.Set(z, Width/YStep-1, x, slice[Width/YStep-1][x]-deltaHta*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	} else {
		// 需要修改焓的变化到温度变化k映射关系
		c.thermalField.Set(z, Width/YStep-1, x, slice[Width/YStep-1][x]-deltaHta*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	}
}

// 计算right top点的温度变化
func (c *calculatorWithArrDeque) calculatePointRT(deltaT float32, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHrt = deltaHrt * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	//fmt.Println(
	...
	if c.alternating { // 需要修改焓的变化到温度变化的映射关系)
		c.thermalField1.Set(z, Width/YStep-1, Length/XStep-1, slice[Width/YStep-1][Length/XStep-1]-deltaHrt*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	} else {
		c.thermalField.Set(z, Width/YStep-1, Length/XStep-1, slice[Width/YStep-1][Length/XStep-1]-deltaHrt*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	}
}

// 计算右表面点的温度变化
func (c *calculatorWithArrDeque) calculatePointRA(deltaT float32, y, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHra = deltaHra * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	...
	if c.alternating { // 需要修改焓的变化到温度变化的映射关系
		c.thermalField1.Set(z, y, Length/XStep-1, slice[y][Length/XStep-1]-deltaHra*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	} else {
		c.thermalField.Set(z, y, Length/XStep-1, slice[y][Length/XStep-1]-deltaHra*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	}
}

// 计算right bottom点的温度变化
func (c *calculatorWithArrDeque) calculatePointRB(deltaT float32, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHrb = deltaHrb * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	...
	if c.alternating { // 需要修改焓的变化到温度变化的映射关系
		c.thermalField1.Set(z, 0, Length/XStep-1, slice[0][Length/XStep-1]-deltaHrb*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	} else {
		c.thermalField.Set(z, 0, Length/XStep-1, slice[0][Length/XStep-1]-deltaHrb*parameter.Enthalpy2Temp, parameter.TemperatureBottom)
	}
}

// 计算下表面点的温度变化
func (c *calculatorWithArrDeque) calculatePointBA(deltaT float32, x, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHba = deltaHba * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	//fmt.Println(
...
// 计算left bottom点的温度变化
func (c *calculatorWithArrDeque) calculatePointLB(deltaT float32, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHlb = deltaHlb * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	//fmt.Println(
...
// 计算左表面点温度的变化
func (c *calculatorWithArrDeque) calculatePointLA(deltaT float32, y, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHla = deltaHla * (2 * deltaT / parameter.Density[index]) / enthalpyScale
	//fmt.Println(
...
// 计算内部点的温度变化
func (c *calculatorWithArrDeque) calculatePointIN(deltaT float32, x, y, z int, slice *casting_machine.ItemType, parameter *steel_type.Parameter) {
	...
	deltaHin = deltaHin * (2 * deltaT / parameter.Density[index]) / enthalpyScale
