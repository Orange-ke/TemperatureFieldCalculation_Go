...
func (e *executorBaseOnSlice) traverseSpirally(t task, c *calculatorWithArrDeque) {
	...
	c.Field.TraverseSpirally(t.start, t.end, func(z int, item *casting_machine.ItemType) {
		// 跳过为空的切片， 即值为-1
		if item[0][0] == -1 {
			return
		}
		left, right, top, bottom := 0, casting_machine.Length/casting_machine.XStep-1, 0, casting_machine.Width/casting_machine.YStep-1 // 每个切片迭代时需要重置
		// parameter set
...
func (e *executorBaseOnBlock) calculateCase1(t task, c *calculatorWithArrDeque) {
	...
	c.Field.Traverse(func(z int, item *casting_machine.ItemType) {
		...
		c.calculatePointLT(t.deltaT, z, item, parameter)
		count++
		for i := 1; i < casting_machine.Length/casting_machine.XStep/2; i++ {
			c.calculatePointTA(t.deltaT, i, z, item, parameter)
			count++
		}
		for j := casting_machine.Width / casting_machine.YStep / 2; j < casting_machine.Width/casting_machine.YStep-1; j++ {
			c.calculatePointLA(t.deltaT, j, z, item, parameter)
			count++
		}
		for j := casting_machine.Width/casting_machine.YStep - 1 - c.edgeWidth; j < casting_machine.Width/casting_machine.YStep-1; j++ {
			for i := 1; i < 1+c.edgeWidth; i++ {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
		for j := casting_machine.Width / casting_machine.YStep / 2; j < casting_machine.Width/casting_machine.YStep-1-c.edgeWidth; j++ {
			for i := 1; i < 1+c.edgeWidth; i++ {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		...
		for j := casting_machine.Width/casting_machine.YStep - 1 - c.edgeWidth; j < casting_machine.Width/casting_machine.YStep-1; j++ {
			for i := 1 + c.edgeWidth; i < casting_machine.Length/casting_machine.XStep/2; i = i + 1 {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
		for j := casting_machine.Width / casting_machine.YStep / 2; j < casting_machine.Width/casting_machine.YStep-1-c.edgeWidth; j = j + c.step {
			for i := 1 + c.edgeWidth; i < casting_machine.Length/casting_machine.XStep/2; i = i + c.step {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
...
func (e *executorBaseOnBlock) calculateCase2(t task, c *calculatorWithArrDeque) {
	...
	c.Field.Traverse(func(z int, item *casting_machine.ItemType) {
		...
		c.calculatePointRT(t.deltaT, z, item, parameter)
		count++
		for i := casting_machine.Length / casting_machine.XStep / 2; i < casting_machine.Length/casting_machine.XStep-1; i++ {
			c.calculatePointTA(t.deltaT, i, z, item, parameter)
			count++
		}
		for j := casting_machine.Width / casting_machine.YStep / 2; j < casting_machine.Width/casting_machine.YStep-1; j++ {
		...
		for j := casting_machine.Width/casting_machine.YStep - 1 - c.edgeWidth; j < casting_machine.Width/casting_machine.YStep-1; j++ {
			for i := casting_machine.Length/casting_machine.XStep - 1 - c.edgeWidth; i < casting_machine.Length/casting_machine.XStep-1; i++ {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
		...
		for j := casting_machine.Width/casting_machine.YStep - 1 - c.edgeWidth; j < casting_machine.Width/casting_machine.YStep-1; j++ {
			for i := casting_machine.Length / casting_machine.XStep / 2; i < casting_machine.Length/casting_machine.XStep-1-c.edgeWidth; i = i + 1 {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
		for j := casting_machine.Width / casting_machine.YStep / 2; j < casting_machine.Width/casting_machine.YStep-1-c.edgeWidth; j = j + c.step {
			for i := casting_machine.Length / casting_machine.XStep / 2; i < casting_machine.Length/casting_machine.XStep-1-c.edgeWidth; i = i + c.step {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
...
func (e *executorBaseOnBlock) calculateCase3(t task, c *calculatorWithArrDeque) {
	...
	c.Field.Traverse(func(z int, item *casting_machine.ItemType) {
		...
		for j := 1; j < casting_machine.Width/casting_machine.YStep/2; j++ {
			c.calculatePointRA(t.deltaT, j, z, item, parameter)
			count++
		}
		...
		for j := 1 + c.edgeWidth; j < casting_machine.Width/casting_machine.YStep/2; j++ {
			for i := casting_machine.Length/casting_machine.XStep - 1 - c.edgeWidth; i < casting_machine.Length/casting_machine.XStep-1; i++ {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		...
		for j := 1; j < 1+c.edgeWidth; j++ {
			for i := casting_machine.Length / casting_machine.XStep / 2; i < casting_machine.Length/casting_machine.XStep-1-c.edgeWidth; i++ {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
		for j := 1 + c.edgeWidth; j < casting_machine.Width/casting_machine.YStep/2; j = j + c.step {
			for i := casting_machine.Length / casting_machine.XStep / 2; i < casting_machine.Length/casting_machine.XStep-1-c.edgeWidth; i = i + c.step {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
...
func (e *executorBaseOnBlock) calculateCase4(t task, c *calculatorWithArrDeque) {
	...
	c.Field.Traverse(func(z int, item *casting_machine.ItemType) {
		...
		for j := 1; j < 1+c.edgeWidth; j++ {
			for i := 1 + c.edgeWidth; i < casting_machine.Length/casting_machine.XStep/2; i++ {
				c.calculatePointIN(t.deltaT, i, j, z, item, parameter)
				count++
			}
		}
